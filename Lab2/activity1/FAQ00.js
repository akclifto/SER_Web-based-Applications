...
class FAQ {
  ...
  writeQA(question, answer, tags, author, date) {
    ...
    for (let i in writes) {
      if (writes[i] === "" || writes[i] === undefined) {
        console.log(
          "writeQA: ",
          "Write not stored. Empty or undefined parameters."
        );
  ...
  filter(filterParams) {
    ...
    if (filterParams.author) {
      filteredQA = filteredQA.filter((qa) =>
        qa.author.toLowerCase().includes(filterParams.author.toLowerCase())
      );
    }
    if (filterParams.dateRange) {
      filteredQA = filteredQA.filter((qa) =>
        qa.date.includes(filterParams.dateRange)
      );
    }
    if (filterParams.tags) {
      filteredQA = filteredQA.filter((qa) =>
        qa.tags.toLowerCase().includes(filterParams.tags.toLowerCase())
      );
    }
  ...
  restoreBackupFile() {
    const options = { encoding: "utf8", flag: "r" };
    try {
      let qas = readFileSync(__dirname + "/QA_backup.json", options);
