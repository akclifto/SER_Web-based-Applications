    ...
    processFormData(req, res, function (formData) {
      ...
      if (formData.login) {
        ...
        // login is valid, get userRole, set appropriate login
          ...
          serverLog(
            "Logging in user " + formData.username + " as " + formData.role
          );
          homePage(req, res, formData, faq);
        } else {
          loginInvalid(res);
  ...
  if (role === "student") {
    ...
    if (items[0] === undefined) {
      display =
        ...
        "<br/><br/>" +
        "<b>No results from search filter. <b>";
      return display;
    }
    ...
    let addToPage = items.map((item) => {
      let itemContent =
        ...
        '<input type="hidden" name="itemQuestion" value=' +
        JSON.stringify(item.question) +
        ">" +
        '<input type="hidden" name="itemAnswer" value=' +
  ...
  if (qaCheck) {
    ...
    try {
      readFile("./Lab2/html/home.html", function (err, content) {
        ...
        const error =
          "One or more fields are missing. Please fill out all fields before saving.";
        content = content.toString().replace("{content}", page);
        content = content
          .toString()
          .replace("{question}", formData.itemQuestion);
...
function checkLogin(req, postData) {
  ...
  for (let i in fake.db) {
    if (
      postData.username === fake.db[i].username &&
...
function editContentPage(formData) {
  let page =
    ...
    JSON.stringify(formData.itemQuestion) +
    ">" +
  ...
  try {
    readFile("./Lab2/html/login.html", function (err, content) {
      ...
      if (req.headers.cookie) {
        ...
        username = username.split("=");
        const greeting =
          "Welcome back " + username[1] + ", please enter your password.";
...
function logout(req, res, resultFunc) {
  ...
  try {
    readFile("./Lab2/html/login.html", function (err, content) {
      ...
      content = content
        .toString()
        .replace("{login}", "You have been logged out.");
      content = content.toString().replace("{Username}", username[1]);
      resultFunc(content);
    });
  } catch (err) {
  ...
  if (qaCheck) {
    ...
    try {
      readFile("./Lab2/html/home.html", function (err, content) {
        ...
        const error =
          "One or more fields are missing. Please fill out all fields to add a question.";
        content = content.toString().replace("{content}", page);
        content = content.toString().replace("{errorText}", error);
        res.write(content);
        res.end();
